...
For this class, we will define deployments for our FastAPI application and its associated components, as deployments
...
``username/hello-fastapi:dev`` has a tag of ``dev``, in this case indicating that the image was built from the ``dev`` branch
