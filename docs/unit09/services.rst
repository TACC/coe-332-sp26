...
We found the IP address for our FastAPI container, but if we try to communicate with it from our Jetstream VMs, 
...
and port. Let's define a service for our hello-fastapi deployment.
...
Let's look at the ``spec`` description for this service.
  ...
  * ``selector`` -- This tells k8s what pod containers to match for the service. Here we are using a label,
    ``app: hello-fastapi``, which means k8s will link all pods with this label to our service. Note that it is important that
...
1. A second ``Service`` object of type ``NodePort`` which selects your deployment using the 
   deployment label and exposes your FastAPI on a public port. 
2. An ``Ingress`` object which specifies a subdomain to make your FastAPI available on and 
...
2. The value of ``app`` in the ``selector`` stanza needs to match the ``app`` label in your 
   ...
   previously. As mentioned before, be sure the selector targets the **label** in your FastAPI
...
.. code-block:: yaml    
   ...
   metadata:
     name: hello-fastapi-ingress
     annotations:
       nginx.ingress.kubernetes.io/ssl-redirect: "false"
   spec:
     ingressClassName: nginx
     rules:
     - host: "username-fastapi.coe332.tacc.cloud"
       http:
           paths:
           - pathType: Prefix
             path: "/"
