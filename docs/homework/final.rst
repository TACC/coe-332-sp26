...
* The front-end REST API must have:
  ...
  * use pydantic data models to describe the objects in the dataset. 
...
* The Redis database must support:
  ...
  * a 'raw data' database holding the Engineering data set using pydantic models
...
instructions for deploying and testing the application both on local hardware (e.g. a Linux VM) and
on a Kubernetes cluster, and instructions for using the application both on local hardware
(e.g. a Linux VM) and at a public endpoint.
...
* Key technologies (e.g. FastAPI, Docker, Kubernetes) are defined at a high level for people who might not know what they are
...
.. code-block:: text
   ...
   ├── kubernetes
   │   ├── prod
   │   │   ├── app-prod-deployment-FastAPI.yml
   │   │   ├── app-prod-deployment-redis.yml
   │   │   ├── app-prod-deployment-worker.yml
   │   │   ├── app-prod-ingress-FastAPI.yml
   │   │   ├── app-prod-pvc-redis.yml
   │   │   ├── app-prod-service-FastAPI.yml
   │   │   ├── app-prod-service-nodeport-FastAPI.yml
   │   │   └── app-prod-service-redis.yml
   │   └── test
   │       ├── app-test-deployment-FastAPI.yml
   │       ├── app-test-deployment-redis.yml
   │       ├── app-test-deployment-worker.yml
   │       ├── app-test-ingress-FastAPI.yml
   │       ├── app-test-pvc-redis.yml
   │       ├── app-test-service-FastAPI.yml
   │       ├── app-test-service-nodeport-FastAPI.yml
...
Send an email to all of the instructors and the TA with the PDF write-up
