...
In this unit, we will begin the work to extend our FastAPI API to enable users to query and analyze data
in our data sets. We want to expose this functionality through our FastAPI API, but there is an issue:
...
in a separate Python program from our FastAPI API. The database will play a central role.

Our basic approach will be:

1. Our dataset will be stored in a database.
2. The user submits a request to a FastAPI endpoint describing some sort of
...
4. The FastAPI API will determine the status of the analysis by querying the database, and it will
...
* Our data needs permanence and we want to be able to stop and start our FastAPI
