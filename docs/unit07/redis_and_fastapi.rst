...
the Redis service from our FastAPI apps. After going through this module, students
should be able to:
  ...
  * Connect to a Redis container from within a FastAPI application.
...
----------------------
Using Redis in our FastAPI apps is identical to using it in the Python shells that we have been using
...
function into a FastAPI API with one route that handles POST, GET, and DELETE requests.
...
A sample template for the FastAPI app can be found `at this link2 <https://raw.githubusercontent.com/TACC/coe-332-sp23/main/docs/unit06/scripts/FastAPI_app_template.py>`_
...
We now have two services to consider: FastAPI and Redis. We need to launch one container for FastAPI,
...
.. code-block:: yaml
    ...
    services:
        ...
        FastAPI-app:
            ...
            depends_on:
                - redis-db
            image: username/ml_FastAPI_app:1.0
            ports:
                - 5000:5000


One thing to note is a new specification under the ``FastAPI-app`` service called '``depends_on``'. 
This ensures that the ``redis-db`` service is up before starting the ``FastAPI-app`` service. 
...
One slight modification to the FastAPI app is required to enable container-to-container
...
``redis-db``. Specifically, in your containerized FastAPI app, establish a Redis client like:
...
.. code-block:: console
    ...
    Creating redis_FastAPI-app_1 ... done                                                                                                    
    ...
    193f057687a8   username/ml_FastAPI_app:1.0   "python3 /ml_FastAPI_a…"   3 seconds ago   Up 2 seconds   0.0.0.0:5000->5000/tcp, :::5000->5000/tcp   redis_FastAPI-app_1
...
.. code-block:: console
   ...
   Stopping redis_FastAPI-app_1 ... done
   Removing redis_redis-db_1  ... done             
   Removing redis_FastAPI-app_1 ... done                                                                                                    
