...
to the Redis db, and for the user to query the database with a fast api route and
retrieve the image. This would be the general workflow:


1. The user submits a curl request from, e.g., the py-debug pod to the Fast API
2. The Fast API creates a new job entry in the appropriate Redis db, and adds the UUID to the queue
